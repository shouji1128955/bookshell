...
在Docker中，`CMD`和`ENTRYPOINT`可以使用exec格式定义，这是为了提供更好的信号处理和操作系统间兼容性。使用exec格式的`CMD`和`ENTRYPOINT`以JSON数组的形式定义命令，这有助于确保命令以可执行文件的形式运行，而不是通过shell解释。这对于信号处理和与操作系统的交互更加可控，尤其在Linux环境下。
